...
public class Word2VecUseDAO {
	...
	public Binary findEntrenamiento(String id, String wid) {
		...
		try (MongoClient mongoClient = new MongoClient(uri)){
			...
			collection.updateOne(eq("_id", new ObjectId(id)), set("palabrasConsultadas", statics.getInteger("palabrasConsultadas") + 1));
			collection.updateOne(eq("_id", new ObjectId(id)), set("peticionesWord2VecUse", statics.getInteger("peticionesWord2VecUse") + 1));
